...
Changing the key requires a signed `SETUP_SIGNING` message. Disabling signing requires physical SD card access.
:::

### 2. Secure Physical Access

- **SD card**: The signing key is stored at `/mavlink/mavlink-signing-key.bin`. Anyone with physical access to the SD card can read, modify, or remove the key file. This is the only way to disable signing.
- **USB ports**: USB is the only link that accepts `SETUP_SIGNING` messages, but the message must be signed when signing is active. USB does not bypass signing.
...
- **Key management**: There is no automatic key rotation. Keys must be reprovisioned manually via a signed `SETUP_SIGNING` message over USB.
